...
import SadBoyImage from '../images/sadBoy.png';

window.F1DataVis = window.F1DataVis || {};
var F1DataVis = window.F1DataVis;
...
F1DataVis.paraCoorder = function (svgParent, visualizer) {
    var self = this,
        ...
        _buttonYPos,
        _apologyMessage = [
            F1DataVis.Texts.ApologyMessage1,
            '',
            F1DataVis.Texts.ApologyMessage2,
        ],
        _initializeClipping = function () {
            ...
            _clipRect
                ...
                .attr(
                    'height',
                    self.height - _clippingProps.top - _clippingProps.bottom
                )
                .attr(
                    'width',
                    self.width - _clippingProps.left - _clippingProps.right
                );
            _paracoordHolder.attr(
                'clip-path',
                'url(#' + F1DataVis.IdStore.paracoordClipper + ')'
            );
        },
        _translateSeasonOut = function (negator) {
            self.seasonalGrp[_displayedYear]
                ...
                .attr(
                    'transform',
                    'translate(' + self.width * 1.5 * negator + ',0)'
                );
        },
        ...
        //     if (
        //         self.racialParams[_displayedRaceId] &&
        //         self.racialParams[_displayedRaceId].paths
        //     ) {
        //         var length = self.racialParams[_displayedRaceId].paths.length,
        //             i,
        //             path,
        //             pathLength;
        //         for (i = 0; i < length; i++) {
        ...
        //             path.attr('stroke-dasharray', pathLength).attr(
        //                 'stroke-dashoffset',
        //                 pathLength
        //             );
        //         }
        ...
        _onRaceChanged = function (moveToPrev) {
            ...
            if (moveToPrev) {
                ...
                if (
                    _displayedRaceIndex ===
                    self.seasonalParams[_displayedYear].races.length - 1
                ) {
                    return;
                }
                _displayedRaceIndex++;
            ...
            race =
                self.seasonalParams[_displayedYear].races[_displayedRaceIndex];
            self.drawRace(race, negator);
        },
        _moveTooltip = function (event) {
            var splittedId = event.currentTarget.id.split('_'),
                driverId = parseInt(splittedId.pop(), 10),
                tooltipData = F1DataVis.dataHandler.getTooltipData(
                    _displayedRaceId,
                    driverId
                );
    ...
    this.initialize = function (width, height) {
        ...
        _leftButton = new F1DataVis.button(
            _svgParent,
            _leftButtonRestPos,
            _buttonYPos,
            _buttonSize,
        ...
        _rightButton = new F1DataVis.button(
            _svgParent,
            _rightButtonRestPos,
            _buttonYPos,
            _buttonSize,
        ...
        this.tooltip = new F1DataVis.tooltip(
            _paracoordParentGrp,
            F1DataVis.IdStore.tooltip,
            this.width - _marginProps.right,
            this.height - _marginProps.bottom
        );
    };
    ...
    this.drawSeason = function (year) {
        ...
        if (this.seasonalGrp[_displayedYear]) {
            ...
            orderedTeams = teams
                .map((teamId) => F1DataVis.dataHandler.getTeamsByID(teamId))
                .sort((teamA, teamB) => {
            ...
            getXPositionOfRace = d3.scalePoint(d3.range(0, numberOfRaces + 1), [
                _marginProps.left,
                this.width - _marginProps.right,
            ]);

            verticalAxisRange = d3.range(
                _marginProps.top,
                self.height - _marginProps.bottom + 1,
                (self.height - _marginProps.bottom - _marginProps.top) /
                    (numberOfTeams - 1)
            );
            ...
            this.seasonalParams[_displayedYear] = {
                races: races,
                orderedTeams: orderedTeams,
                raceScales: raceScales,
                getXPositionOfRace: getXPositionOfRace,
            };
    ...
    this.drawSeasonAxes = function (races, negator) {
        var getXPositionOfRace =
                this.seasonalParams[_displayedYear].getXPositionOfRace,
        ...
        this.seasonalGrp[_displayedYear]
            ...
            .attr(
                'id',
                (race) => 'Year_' + _displayedYear + '_Round_' + race.round
            )
            .attr(
                'transform',
                (race) => `translate(${getXPositionOfRace(race.round)},0)`
            )
            ...
            .on('mouseover', function () {
                if (_displayingYear === true) {
                    this.classList.add(
                        F1DataVis.IdStore.highlightedElementClass
                    );
                }
            })
            ...
            .each(function (race, index) {
                ...
                if (index === 0) {
                    d3.select(this)
                        ...
                        .call(d3.axisLeft(raceScales.get(race.round)));
                } else {
                    if (index === races.length) {
                        d3.select(this)
                            .attr('class', 'rightAxisLabels')
                            .call(d3.axisRight(raceScales.get(race.round)));
                    } else {
                        d3.select(this)
                            .append('path')
                            .attr(
                                ...
                                d3.line()([
                                    [0, _marginProps.top],
                                    [0, self.height - _marginProps.bottom],
                                ])
                            )
                    ...
                    d3.select(this)
                        ...
                        .attr(
                            'class',
                            F1DataVis.IdStore.highlightableElementClass
                        )
                ...
                if (index === 0) {
                    textGroup.attr('class', 'axisXLabels first');
                } else {
                    textGroup.attr('class', 'axisXLabels highlightableText');
                }

                for (let i = 0; i < raceNameFormatted.length; i++) {
                    textGroup
                        .append('text')
                        .attr('x', 0)
                        .attr(
                            'y',
                            self.height - _marginProps.bottom + i * 10 + 20
                        )
    ...
    this.drawRace = function (race, negator) {
        ...
        if (negator === 0) {
            ...
            this.raceGrp[_displayedRaceId]
                ...
                .attr(
                    'transform',
                    'translate(' + this.width * 1.5 * negator + ',0)'
                );
            translateFromAttr =
                'translate(' + this.width * 1.5 * -negator + ',0)';
            translateToAttr = 'translate(0,0)';
        ...
        if (this.raceGrp[_displayedRaceId]) {
            ...
            driverObjects =
                F1DataVis.dataHandler.getDriverObjByPolePositions(
                    _displayedRaceId
                );
            ...
            this.racialParams[_displayedRaceId] = racialParams = {
                ...
                numberOfLaps: 0,
                lapScales: [],
                getXPositionOfLap: undefined,
                paths: [],
                leftButtonPos: _leftButtonRestPos,
                rightButtonPos: _rightButtonRestPos,
                dashBoardBound: {},
                pathsDrawn: false,
            };
            if (driverObjects === undefined) {
                ...
                if (numberOfDrivers > driverObjects.length) {
                    for (driverId in drivers) {
                        if (driverIds.indexOf(parseInt(driverId, 10)) === -1) {
                            driverObjects.push(
                                F1DataVis.dataHandler.getDriversByID(
                ...
                getXPositionOfLap = d3.scalePoint(lapRange, [
                    _marginProps.left,
                    this.width - _marginProps.right,
                ]);
            ...
            var raceNametext = this.raceGrp[_displayedRaceId]
                    .append('g')
                    .attr('id', 'raceNameGrp')
                    .append('text')
                    .attr('x', self.width / 2)
            ...
            racialParams.leftButtonPos =
                self.width / 2 -
                textBBox.width / 2 -
                _buttonSize -
                _buttonToTextGap;
            racialParams.rightButtonPos =
                self.width / 2 + textBBox.width / 2 + _buttonToTextGap;
            racialParams.dashBoardBound.topLeft = {
                x: racialParams.leftButtonPos,
                y: _buttonYPos,
            };
            racialParams.dashBoardBound.topRight = {
                x: racialParams.rightButtonPos + _buttonSize,
                y: _buttonYPos,
            };
        ...
        if (
            _displayedRaceIndex ===
            this.seasonalParams[_displayedYear].races.length - 1
        ) {
            _rightButton.transitionX(_rightButtonRestPos);
        } else {
    ...
    this.drawRaceAxes = function (translateFromAttr, translateToAttr) {
        var lapScales = this.racialParams[_displayedRaceId].lapScales,
            ...
            getXPositionOfLap =
                this.racialParams[_displayedRaceId].getXPositionOfLap,
            data = [{ text: ' LAPS>>>', lap: 0 }].concat(
                d3.range(1, numberOfLaps + 1).map((val) => {
        ...
        this.raceGrp[_displayedRaceId]
            ...
            .attr(
                'id',
                (text) => 'Race' + _displayedRaceId + '_Lap_' + text.lap
            )
            .attr(
                'transform',
                (text) => `translate(${getXPositionOfLap(text.lap)},0)`
            )
            .each(function (text, index) {
                ...
                if (index === 0) {
                    d3.select(this)
                        .attr('class', 'axisLabels')
                        .call(d3.axisLeft(lapScales.get(text.lap)));
                } else {
                    if (index === numberOfLaps) {
                        d3.select(this)
                            .attr('class', 'rightAxisLabels')
                            .call(d3.axisRight(lapScales.get(text.lap)));
                    } else {
                        d3.select(this)
                            .append('path')
                            .attr(
                ...
                var raceNameFormatted = text.text.split(' '),
                    i,
                    textGroup = d3
                        .select(this)
                        .append('g')
                ...
                for (i = 0; i < raceNameFormatted.length; i++) {
                    textGroup
                        ...
                        .attr(
                            'y',
                            self.height - _marginProps.bottom + i * 10 + 15
                        )
                        .attr('text-anchor', 'middle')
                        .text(raceNameFormatted[i]);
                }
    ...
    this.getYCoordinate = function (round, position) {
        return this.seasonalParams[_displayedYear].raceScales.get(round)(
            position
        );
    };
    ...
    this.drawSeasonPaths = function () {
        var races = this.seasonalParams[_displayedYear].races,
            ...
            getXPositionOfRace =
                this.seasonalParams[_displayedYear].getXPositionOfRace,
            getColour = d3
                .scaleOrdinal(d3.schemeCategory10.concat(d3.schemeCategory10))
                .domain(teams.map((team) => team.constructorId));
        this.seasonalGrp[_displayedYear]
            .append('g')
            .attr('id', () => 'Year_' + _displayedYear + '_polylineGroup')
            .selectAll('path')
            .data(teams)
            .join('g')
            .attr(
                ...
                (team) =>
                    'Year_' +
                    _displayedYear +
                    '_polylineGroup_Team_' +
                    team.constructorId
            )
            .on('mouseover', function () {
                this.parentElement.classList.add(
                    F1DataVis.IdStore.dehighlightedGroupClass
                );
            ...
            .each(function (team) {
                var points = d3.range(0, races.length + 1).map((round) => {
                        var x = getXPositionOfRace(round),
                            y,
                            position;
                        if (round === 0) {
    ...
    this.drawRacePaths = function () {
        ...
        if (
            self.racialParams[_displayedRaceId] &&
            this.racialParams[_displayedRaceId].paths.length === 0
        ) {
            ...
            getColour = d3
                .scaleOrdinal(d3.schemeCategory10.concat(d3.schemeCategory10))
                .domain(driverObjects.map((driver) => driver.driverId));
            this.raceGrp[_displayedRaceId]
                .append('g')
                .attr('id', 'group_race_' + _displayedRaceId)
                .selectAll('path')
                ...
                .on('mouseover', function () {
                    ...
                    this.classList.add(
                        F1DataVis.IdStore.highlightablePathClass
                    );
                })
                ...
                .each(function (driver) {
                    var points = d3.range(0, numberOfLaps + 1).map((lap) => {
                            var x = getXPositionOfLap(lap),
                                y,
                                position,
                                statusId;

                            if (lap === 0) {
                                y = self.getYCoordinateOfDriver(
                                    0,
                                    _getDriverName(driver)
                                );
                            } else {
                                if (
                                    drivers[driver.driverId][lap - 1] ===
                                    undefined
                                ) {
                                    if (
                                        drivers[driver.driverId][lap - 2] ===
                                            undefined ||
                                        ((statusId =
                                            F1DataVis.dataHandler.getResultDetails(
                                                _displayedRaceId,
                                                driver.driverId
                                            ).statusId) > 10 &&
                                            statusId < 20)
    ...
    this.onRaceClicked = function (event) {
        var round = parseInt(
                event.currentTarget.id.split('Round_')[1].split('_')[0],
                10
            ),
            race;
        if (round === 0) {
        ...
        race = F1DataVis.dataHandler.getRaceByRoundInYear(
            _displayedYear,
            round
        );
