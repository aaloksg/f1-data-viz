...
import AlkIntroPhoto from '../images/introPhotoAlk.png';
import AnkIntroPhoto from '../images/introPhotoAnk.png';
...
F1DataVis.introducer = function (introGrp, logoStyles, visualizer) {
    var _visualizer = visualizer,
        ...
        _initPositions = function () {
            ...
            _textPositions.leftTextX =
                _logoButtonPos.left - _textPositions.margin.right;
            _textPositions.leftTextAnchor = 'end';
            _textPositions.rightTextX =
                _logoButtonPos.left +
                _logoButtonPos.width +
                _textPositions.margin.left;
        ...
        _createTexts = function () {
            ...
            F1DataVis.Texts.Quote = [
                '"Races are won at the track.',
                'Championships are won at the factory."',
                '- Mercedes( 2019 )',
            ];
            F1DataVis.Texts.LeftIntro = [
                'Aalok Shashidhar Gokhale',
                '121872',
                'aalok.shashidhar.gokhale@uni-weimar.de',
                'Bauhaus-Universität',
                'Weimar',
            ];
            F1DataVis.Texts.RightIntro = [
                'Ankith Kodanda',
                '121983',
                'ankith.kodanda@uni-weimar.de',
                'Bauhaus-Universität',
                'Weimar',
            ];
        },
        _logoGradAnim2 = function () {
            d3.select('#' + F1DataVis.IdStore.LogoNormal + '_1').attr(
                'stop-color',
                'black'
            );
            d3.select('#' + F1DataVis.IdStore.LogoNormal + '_3').attr(
                'stop-color',
                'midnightblue'
            );
        ...
        _checkMousePosition = function (event) {
            var distance = Math.sqrt(
                    Math.pow(event.clientX - _logoCenter.x, 2) +
                        Math.pow(event.clientY - _logoCenter.y, 2)
                ),
                bufferRadius = _logoButtonPos.width / 2;
    ...
    this.draw = function () {
        ...
        _logoButtonPos.width =
            logoStyles.largeWidth +
            _logoButtonMargins.left +
            _logoButtonMargins.right;
        _logoButtonPos.height =
            logoStyles.largeHeight +
            _logoButtonMargins.top +
            _logoButtonMargins.bottom;
        ...
        _textPositions.topTextY =
            _logoButtonPos.top - _textPositions.margin.bottom - totalHeight;
        _topTextGrp.attr(
            'transform',
            'translate(' +
        ...
        for (i = 0; i < length; i++) {
            textElements[i].attr('x', 0).attr('y', yPos);
        ...
        textHeight = textElements[0]._groups[0][0].getBBox().height;
        totalHeight = length * textHeight + textHeight * 0.5 * (length - 1);
        _textPositions.leftTextY =
            _logoButtonPos.top + _logoButtonPos.height / 2 - totalHeight / 2;
        _leftTextGrp.attr(
            'transform',
            'translate(' +
                _textPositions.leftTextX +
        ...
        _rightTextGrp = _introGrp.append('g').attr('id', 'DataCreditTextGroup');
        for (i = 0; i < length; i++) {
            textElements.push(
                _rightTextGrp
                    .append('a')
        ...
        textHeight = textElements[0]._groups[0][0].getBBox().height;
        totalHeight = length * textHeight + textHeight * 0.5 * (length - 1);
        _textPositions.clickToStartTextY =
            _logoButtonPos.top +
            _logoButtonPos.height +
            _textPositions.margin.top / 4;
        _clickToStartTextGrp.attr(
            'transform',
            'translate(' +
                _textPositions.clickToStartTextX +
                ',' +
                _textPositions.clickToStartTextY +
                ')'
        );
        yPos = textHeight * 0.75;
        _textPositions.clickToStartTextGap =
            textHeight * _textPositions.textGapFactor;
        for (i = 0; i < length; i++) {
            textElements[i].attr('x', 0).attr('y', yPos);
        ...
        _textPositions.leftIntroTextY = _textPositions.rightIntroTextY =
            _textPositions.clickToStartTextY + yPos + _textPositions.margin.top;
        _textPositions.introImageSize =
            _visualizer.height -
            _textPositions.leftIntroTextY -
            _textPositions.margin.top;
        _textPositions.leftIntroTextX =
            _visualizer.width / 2 -
            _textPositions.introImageSize -
            _textPositions.introImageMargin / 2 -
            _textPositions.margin.right;
        _textPositions.rightIntroTextX =
            _visualizer.width / 2 +
            _textPositions.introImageSize +
            _textPositions.introImageMargin / 2 +
            _textPositions.margin.left;
        ...
        _introGrp
            ...
            .attr('d', () =>
                d3.line()([
                    [_visualizer.width / 2, _textPositions.leftIntroTextY],
                    [
                        _visualizer.width / 2,
                        _textPositions.leftIntroTextY +
                            _textPositions.introImageSize,
        ...
        _leftIntroGrp = _introGrp.append('g').attr('id', 'LeftIntroGroup');
        _rightIntroGrp = _introGrp.append('g').attr('id', 'RightIntroGroup');
        ...
        anchor = _rightIntroAnchorGrp
            ...
            .attr(
                'href',
                'https://www.linkedin.com/in/ankith-kodanda-3365b3103'
            )
            .attr('target', '_blank');
        anchor
            .append('rect')
        ...
        anchor
            ...
            .attr(
                'x',
                _visualizer.width / 2 + _textPositions.introImageMargin / 2
            )
            .attr('y', _textPositions.leftIntroTextY)
            .attr('height', _textPositions.introImageSize)
            .attr('width', _textPositions.introImageSize)
            .attr('href', AnkIntroPhoto);
        ...
        _textPositions.leftIntroTextY =
            _textPositions.leftIntroTextY +
            _textPositions.introImageSize / 2 -
            totalHeight / 2;

        _leftIntroTextGrp.attr(
            'transform',
            'translate(' +
                _textPositions.leftIntroTextX +
                ',' +
        ...
        yPos = textHeight * 0.75;
        _textPositions.leftIntroTextGap =
            textHeight * _textPositions.textGapFactor;
        for (i = 0; i < length; i++) {
            textElements[i].attr('x', 0).attr('y', yPos);
        ...
        textHeight = textElements[0]._groups[0][0].getBBox().height;
        totalHeight = length * textHeight + textHeight * 0.5 * (length - 1);
        _textPositions.rightIntroTextY =
            _textPositions.rightIntroTextY +
            _textPositions.introImageSize / 2 -
            totalHeight / 2;

        _rightIntroTextGrp.attr(
            'transform',
            'translate(' +
                _textPositions.rightIntroTextX +
                ',' +
        ...
        yPos = textHeight * 0.75;
        _textPositions.rightIntroTextGap =
            textHeight * _textPositions.textGapFactor;
        for (i = 0; i < length; i++) {
            textElements[i].attr('x', 0).attr('y', yPos);
    ...
    this.startStopAnim = function (doAnimation) {
        ...
        if (_doAnimation) {
            _logoGradAnim2();
            d3.select(_visualizer.parentSvg).on(
                'mousemove',
                _checkMousePosition
            );
        } else {
