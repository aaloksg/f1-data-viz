...
F1DataVis.dataHandler.initializeData = function () {
    ...
    if (F1DataVis.data.races) {
        ...
        for (item in F1DataVis.data.racesByYear) {
            F1DataVis.data.racesByYear[item].sort((raceA, raceB) => {
    ...
    if (F1DataVis.data.qualifying) {
        ...
        for (item in F1DataVis.data.polePositionsByRaceId) {
            F1DataVis.data.polePositionsByRaceId[item].sort((posA, posB) => {
    ...
    for (i = 0; i < length; i++) {
        F1DataVis.data.statusesById[F1DataVis.data.status[i].statusId] =
            F1DataVis.data.status[i].status;
    }
...
F1DataVis.dataHandler.getRaceIdsInYear = function (year) {
    var races = F1DataVis.data.racesByYear[year],
        raceIds = [],
        length = races.length,
        i;
    for (i = 0; i < length; i++) {
    ...
    if (F1DataVis.data.constructorStandings) {
        for (i = 0; i < length; i = i + jump) {
            ...
            if (raceIds.indexOf(raceId) > -1) {
                ...
                if (
                    F1DataVis.data.constructorStandingsByRaceId[raceId] ===
                    undefined
                ) {
                    if (i !== 0) {
                        lowerBound = i - jump;
                    ...
                    upperBound = j;
                    F1DataVis.data.constructorStandingsByRaceId[raceId] =
                        F1DataVis.data.constructorStandings.slice(
                            lowerBound,
                            upperBound
                        );
                    i = upperBound - jump;
...
F1DataVis.dataHandler.createDetailedPositions = function (year) {
    var racesByYear = F1DataVis.data.racesByYear,
        races,
        i,
        length,
        raceId,
    ...
    for (i = 0; i < length; i++) {
        ...
        for (j = 0; j < length2; j++) {
            object[races[i].round][constructorStandings[j].constructorId] =
                constructorStandings[j].position;
        }
    }
};
...
F1DataVis.dataHandler.getTeamsInSeason = function (year) {
    ...
    if (F1DataVis.data.teamIdsByYear[year]) {
        ...
        for (raceIndex = 0; raceIndex < racesLength; raceIndex++) {
            constructorStandings =
                F1DataVis.data.constructorStandingsByRaceId[
                    races[raceIndex].raceId
                ];
            noOfStandings = constructorStandings.length;
            for (
                standingIndex = 0;
                standingIndex < noOfStandings;
                standingIndex++
            ) {
                constructorId =
                    constructorStandings[standingIndex].constructorId;
...
F1DataVis.dataHandler.getLapsByRaceId = function (raceId) {
    ...
    if (F1DataVis.data.laptimesByRaceId[raceId] !== undefined) {
        ...
        laps = F1DataVis.data.laptimesByRaceId[raceId] =
            F1DataVis.data.lapTimes.filter(
                (laptime) => laptime.raceId === raceId
            );
    }
    return laps;
};
...
F1DataVis.dataHandler.getDriverObjByPolePositions = function (raceId) {
    ...
    return F1DataVis.data.polePositionsByRaceId[raceId].map((item) =>
        F1DataVis.dataHandler.getDriversByID(item.driverId)
    );
};
...
F1DataVis.dataHandler.getResultDetails = function (raceId, driverId) {
    ...
    if (F1DataVis.data.resultByRaceDriver[raceId + '_' + driverId]) {
        ...
        for (i = 0; i < length; i++) {
            if (
                results[i].raceId === raceId &&
                results[i].driverId === driverId
            ) {
                F1DataVis.data.resultByRaceDriver[raceId + '_' + driverId] =
                    result = results[i];
                break;
            }
        }
...
F1DataVis.dataHandler.getTooltipData = function (raceId, driverId) {
    var tooltipData, result, teamInfo, driverInfo, status;
